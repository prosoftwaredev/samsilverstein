...
module.exports = function IndexModule(pb) {
    ...
    /**
     * Index The home page controller of the portfolio(RTL) theme.
     * @class Index
    ...
    Index.prototype.render = function(cb) {
        ...
        TopMenu.getTopMenu(self.session, self.ls, options, function(themeSettings, navigation, accountButtons) {
            TopMenu.getBootstrapNav(navigation, accountButtons, function(navigation, accountButtons) {
                ...
                pluginService.getSettings('portfolio-rtl', function(err, portfolioRtlSettings) {
                    var homePageKeywords = '';
                    var homePageDescription = '';
                    for(var i = 0; i < portfolioRtlSettings.length; i++) {
                        switch(portfolioRtlSettings[i].name) {
                            case 'home_page_keywords':
                                homePageKeywords = portfolioRtlSettings[i].value;
                                break;
                            case 'home_page_description':
                                homePageDescription = portfolioRtlSettings[i].value;
                                break;
                            default:
                                break;
                        }
                    ...
                    self.ts.setTheme('portfolio-rtl');
                    self.ts.load('landing_page', function(err, template) {
                        ...
                        var dao = new pb.DAO();
                        dao.q('portfolio_rtl_theme_settings', opts, function(err, settings) {
                            if (util.isError(err)) {
