  ...
  */
  ...
  util.inherits(Blog, pb.BaseController);
   ...
   Blog.prototype.initSync = function(/*context*/) {
        ...
        this.contentService = new pb.ContentService(this.site, true);
        this.PluginService = new pb.PluginService(this.getServiceContext());
        this.commentService = new pb.CommentService(this.getServiceContext());
		this.mservice = new pb.MediaService(this.site, true);
    };
  ...
  Blog.prototype.render = function(cb) {
      ...
      contentService.getSettings(function(err, contentSettings) {
          self.gatherData(function(err, data) {
              var articleService = new pb.ArticleService();
                articleService.getMetaInfo(data.content[0], function(metaKeywords, metaDescription, metaTitle, metaThumbnail) {
				  ...
				  pb.log.error(JSON.stringify(data.content));
		...
		//	pb.log.error(JSON.stringify(sideNavItems));
		//		pb.log.error(sideNavItems.length);
				
				var articleMedia = [];
				
				
				self.getmedia('placeholder', function (allmedia) {
				
			if(sideNavItems != null)	 {
				for(i=0; i< sideNavItems.length; i++){
				if(sideNavItems[i].article_media != null && sideNavItems[i].article_media.length > 0) {
					if(allmedia != null) {
					for(j=0; j<allmedia.length; j++) {
					if(allmedia[j]._id == sideNavItems[i].article_media[0]) {
							articleMedia.push(allmedia[j]);                     
  ...
  Blog.prototype.getmedia = function(id, cb) {
 
	this.mservice.get(function(err, allmedia) {
  ...
  Blog.prototype.getSideNavigation = function(articles, cb) {
        ...
        this.PluginService.getSetting('show_side_navigation', 'tonicblog-pencilblue', function(err, showSideNavigation) {
			...
			pb.log.error("topics" + JSON.stringify(topics));
			pb.log.error("articleIDs" + JSON.stringify(articleIDs));
            ...
            self.siteQueryService.q('article', opts, function(err, relatedArticles) {
				pb.log.error("relatedarticles" + JSON.stringify(relatedArticles));
