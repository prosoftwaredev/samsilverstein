...
module.exports = function (pb) {
  ...
  util.inherits(QuestionFormController, pb.BaseController);

  var SUB_NAV_KEY = 'question_form';

  QuestionFormController.prototype.render = function (cb) {
    var self = this;
    var vars = this.pathVars;

    questionService.getQuestionTypeId(function (err, questionTypeId) {
      vars.type_id = questionTypeId;
      ...
      self.gatherData(vars, function (err, data) {
        ...
        self.ts.load('admin/content/questions/question_form', function (err, result) {
          cb({
            content: result
          });
        });
  ...
  QuestionFormController.prototype.gatherData = function (vars, cb) {
    var self = this;
    ...
    var tasks = {
      ...
      navigation: function (callback) {
        callback(null, pb.AdminNavigation.get(self.session, ['content', 'questions'], self.ls));
      },
      ...
      customObject: function (callback) {
        ...
        questionService.getQuestionById(vars.id,
  ...
  QuestionFormController.prototype.loadFieldOptions = function (service, objectType, cb) {
    var self = this;
  ...
  QuestionFormController.getSubNavItems = function (key, ls, data) {
    return [
      {
        name: 'manage_objects',
        title: data.customObject[pb.DAO.getIdField()] ? ls.get('EDIT') + ' ' + data.customObject.name : ls.get(
          'NEW') + ' ' + data.objectType.name + ' ' + ls.get('OBJECT'),
        icon: 'chevron-left',
      ...
      },
      {
        ...
        title: '',
        icon: 'plus',
        href: '/admin/content/questions/' + data.objectType[pb.DAO.getIdField()] + '/new'
      }
    ];
  };

  QuestionFormController.getRoutes = function (cb) {
    var routes = [
      {
        path: "/admin/content/questions/new",
      ...
      , {
        path: "/admin/content/questions/:id",
        access_level: pb.SecurityService.ACCESS_ADMINISTRATOR,
        auth_required: true,
        handler: 'render',
        content_type: 'text/html'
  ...
  pb.AdminSubnavService.registerFor(SUB_NAV_KEY, QuestionFormController.getSubNavItems);
