...
module.exports = function (pb) {
   ...
   * @class ChannelFormController
   * @constructor
   */
  function ChannelFormController() {
  ...
  util.inherits(ChannelFormController, ss_baseController);

  var SUB_NAV_KEY = 'channel_form';

  ChannelFormController.prototype.render = function (cb) {
    var self = this;
    ...
    this.gatherData(vars, function (err, data) {
          ...
          if (custObjField.field_type === 'child_objects') {
            ...
            for (var j = 0; j < data.customObject[custObjField.name].length; j++) {
              ...
              for (var s = 0; s < custObjField.available_objects.length; s++) {
                ...
                if (pb.DAO.areIdsEqual(associatedObj[pb.DAO.getIdField()],
                    availableObj[pb.DAO.getIdField()])) {
      ...
      self.setPageName(self.customObject[pb.DAO.getIdField()] ? self.customObject.name : self.ls.get(
        'NEW') + ' ' + self.objectType.name + ' ' + self.ls.get('OBJECT'));

      self.ts.registerLocal('angular_objects', new pb.TemplateValue(angularObjects, false));

      self.ts.load('admin/content/channel/channel_form', function (err, result) {
        cb({
          content: result
        });
      });
  ...
  ChannelFormController.prototype.gatherData = function (vars, cb) {
    var self = this;
    ...
    var tasks = {
      ...
      objectType: function (callback) {
        self.cos.loadTypeById(vars.type_id, function (err, objectType) {
          if (util.isError(err)) {
            callback(err, customObject);
            return;
          }

          self.loadFieldOptions(self.cos, objectType, callback);
        });
      },
      channelsType: function (callback) {
        self.channelService.getAllChennalTypes(callback);
      },
      idEmptyOrg: function (callback) {
        self.organizationService.getIdEmptyOrg(callback);
      },
    ...
    async.series(tasks, function (err, data) {
      var key, field;
      for (key = 0; data.objectType.fields.length > key; key++) {
        field = data.objectType.fields[key];
        if (field.field_type === 'child_objects' || field.field_type === 'peer_object') {
          if ((typeof data.customObject[field.name]) === 'string') {
            data.customObject[field.name] = ''
          }
        }
      }
  ...
  ChannelFormController.prototype.loadFieldOptions = function (service, objectType, cb) {
    var self = this;
      ...
      return function (callback) {
        ...
        dao.q(objectType.fields[key].object_type, query, function (err, availableObjects) {
          ...
          for (var i = 0; i < availableObjects.length; i++) {
            ...
            var descriptor = {
              name: availableObjects[i].name || availableObjects[i].headline || userService.getFormattedName(
                availableObjects[i])
            };
  ...
  ChannelFormController.getSubNavItems = function (key, ls, data) {
    return [
      {
        name: 'manage_objects',
...
//        title: ' New',
//        icon: 'plus',
//        href: '/admin/content/channel/' + data.objectType[pb.DAO.getIdField()] + '/new'
//      }
    ];
  };

  ChannelFormController.getRoutes = function (cb) {
    var routes = [
      {
        path: "/admin/content/channel/:type_id/new",
...
//      , {
//        path: "/admin/content/channel/copy/:id",
//        access_level: pb.SecurityService.ACCESS_ADMINISTRATOR,
//        auth_required: true,
//        handler: 'channelCopy',
//        content_type: 'text/html'
//      }
      , {
        path: "/admin/content/channel/:type_id/:id",
        access_level: pb.SecurityService.ACCESS_ADMINISTRATOR,
        auth_required: true,
        handler: 'render',
        content_type: 'text/html'
  ...
  pb.AdminSubnavService.registerFor(SUB_NAV_KEY, ChannelFormController.getSubNavItems);

  //exports
  return ChannelFormController;
};
