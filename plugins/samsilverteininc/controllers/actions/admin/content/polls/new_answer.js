...
module.exports = function (pb) {
  ...
  util.inherits(NewAnswerPoll, pb.BaseController);

  NewAnswerPoll.prototype.save = function (cb) {
    var self = this;

    this.getJSONPostParams(function (err, post) {
      post.author = ('author' in post) ? post.author : self.session.authentication.user_id;

      self.pollService.pollAnswersService.newAnswer(post, function (err, response) {
        if (err) {
          return cb({
            code: 500,
  ...
  NewAnswerPoll.getRoutes = function (cb) {
    var routes = [{
        method: 'post',
        path: "/actions/admin/content/polls/answer/new",
