   ...
   this.initDBConnections = function (cb) {
      ...
      pb.dbm.getDb(pb.config.db.name, function (err, result) {
         if (util.isError(err)) {
            ...
            return cb(new Error("Failed to establish a connection to: " + pb.config.db.name),
                false);
         }
      ...
      try {
         if (pb.config.server.ssl.enabled) {
            ...
            pb.handOffServer.listen(pb.config.server.ssl.handoff_port, function () {
               pb.log.info('PencilBlue: Handoff HTTP server running on port: %d',
                   pb.config.server.ssl.handoff_port);
            });
         }
         else {
         ...
         var onServerStartError = function (err) {
            err.message = util.format(
                "Failed to start HTTP server on PORT=[%s] binding to SITE_IP=[%s]: %s",
                pb.config.sitePort, pb.config.siteIP, err.message);
            cb(err, false);
         };
         pb.server.once('error', onServerStartError);
         pb.server.listen(pb.config.sitePort, pb.config.siteIP, function () {
            pb.log.info('PencilBlue: %s running at site root [%s] on port [%d]',
                pb.config.siteName, pb.config.siteRoot, pb.config.sitePort);
